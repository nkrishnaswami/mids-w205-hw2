...
class Collector(object):
    ...
    def _process(self, method, query_terms, query_ops, page_limit):
        # closing ensures any files being written get flushed/closed.
        with closing(
                FilteringFacet(
                    RegexMatcher('(' + '|'.join(query_terms) + ')'),
                    lambda key: self.make_sink(key))) as facet:
            # set up progress bar
            with _CollectionProgress(method, query_ops) as progress:
                method(query_terms, query_ops, page_limit, progress, facet)
